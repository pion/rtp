...
func (p *AV1Payloader) Payload(mtu uint16, payload []byte) (payloads [][]byte) {
	...
	for offset := 0; offset < len(payload); {
		...
		// nolint:gosec // G104 we validate the size of the payload
...
func (p *AV1Payloader) appendOBUPayload(
	...
	currentPaylod := len(payloads) - 1
	freeSpace := 0
	if currentPaylod >= 0 {
		freeSpace = mtu - len(payloads[currentPaylod])
	}

	if currentPaylod < 0 || freeSpace <= 0 || startWithNewPacket {
	...
	case shouldUseWField:
		payloads[currentPaylod] = append(payloads[currentPaylod], obuPayload[:toWrite]...)
		payloads[currentPaylod][0] |= byte(currentOBUCount<<av1WBitshift) & av1WMask
	case freeSpace >= 2:
		...
		payloads[currentPaylod] = append(payloads[currentPaylod], lengthField...)
		payloads[currentPaylod] = append(payloads[currentPaylod], obuPayload[:toWrite]...)
	default:
	...
	for remaining > 0 {
		...
		if toWrite != 0 {
			payloads[currentPaylod-1][0] |= av1YMask
			payload[0] |= av1ZMask
		}
		...
		// Single OBU fragment.
		if isLast || toWrite != remaining {
			payloads[currentPaylod][0] |= 1 << av1WBitshift
		} else {
