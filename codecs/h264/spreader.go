...
func (s *Spreader) handleNalTooBigOrFua(cumulRtp [][]byte, seqNum uint16, naluType byte, rtpHeader []byte, nalData []byte) ([][]byte, uint16, error) {
	switch {
...
func (s *Spreader) spreadFua(cumulRtp [][]byte, firtSeqNum uint16, rtpHeader []byte, fua []byte) ([][]byte, uint16, error) {
	seqNum := firtSeqNum
...
func (s *Spreader) spreadSingleNalToFua(cumulRtp [][]byte, firtSeqNum uint16, rtpHeader []byte, nal []byte) ([][]byte, uint16, error) {
	entryMarker := rtpHeader[rtpMPtOffset] & rtpMarkerBitMask
	rtpHeader[rtpMPtOffset] &= ^rtpMarkerBitMask
...
func (s *Spreader) explodeStapA(cumulRtp [][]byte, firtSeqNum uint16, rtpHeader []byte, stapa []byte) ([][]byte, uint16, error) {
	entryMarker := rtpHeader[rtpMPtOffset] & rtpMarkerBitMask
	rtpHeader[rtpMPtOffset] &= ^rtpMarkerBitMask
	lenRtpHeader := len(rtpHeader)
