...
func testAV1TestRun(t *testing.T, tests []testAV1Tests) {
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			assert.Equalf(t,
				len(test.OutputPayloads),
				len(result),
				"Expected %d payloads but got %d", len(test.OutputPayloads), len(result),
			)

			for i := range result {
				assert.Equalf(t,
					test.OutputPayloads[i],
					result[i],
					"Expected %v but got %v for payload #%d", test.OutputPayloads[i], result[i], i+1,
				)
			}
		})
	}
