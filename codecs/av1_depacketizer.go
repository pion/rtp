...
// Unmarshal parses an AV1 RTP payload into its constituent OBU elements with obu_size_field,
...
func (d *AV1Depacketizer) Unmarshal(payload []byte) (buff []byte, err error) {
	...
	for offset := 1; offset < len(payload); obuOffset++ {
		...
		if obuHeader.Type != obu.OBUTemporalDelimiter && obuHeader.Type != obu.OBUTileList {
			...
			// But we still check it to be sure, if we get obu size we just use that instead of calculating it.
